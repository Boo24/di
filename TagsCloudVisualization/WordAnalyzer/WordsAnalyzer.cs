...
namespace TagsCloudVisualization.WordAnalyzer
{
    public class WordsAnalyzer
    {
        ...
        public WordsAnalyzer(IBlackList badWords, IEnumerable<IWordsFilter> filters, IEnumerable<IWordConverter> converters)
        {
            blackList = badWords;
            this.filters = filters;
            this.converters = converters;
        }
        public (IEnumerable<Word> sortedWords, int minCount, int maxCount) Analyze(IEnumerable<string> words, int wordsCount)
        {
        ...
        private IEnumerable<Word> ApplyFilters(IEnumerable<Word> allwords) => allwords.Where(w => filters.All(f => f.CheckWord(w)));
